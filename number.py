...
# Функция возврвщающая случайное число от 1 до 100
...
# Этот хэндлер будет срабатывать на команду "/ranked"
@dp.message(Command(commands=['ranked']))
async def process_stat_command(message: Message):
    users_ranked = []
    ranked_list = sorted(users.values(), key=lambda x: -x['wins'])
